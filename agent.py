...
- The `url` field in the submission JSON MUST be the FULL URL of the page you are currently solving (e.g. https://tds-llm-analysis.s-anand.net/demo2). NEVER use a relative path like /demo2.
- NEVER shorten or modify URLs. Always submit the full absolute URL (e.g. https://example.com/quiz/1, NOT /quiz/1).
- The submission JSON MUST always have the fields: `answer`, `url`, `email`, `secret`.
- The `answer` field contains the solution. Do not use other field names like `key` or `result` unless explicitly told the JSON field name is different.
...
- Email: {EMAIL}
- Email: {EMAIL}
- Secret: {SECRET} (MUST be included in the JSON payload for every submission)
